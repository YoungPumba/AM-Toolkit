...
try {
    ...
    $expect(count($participants) === 1 && $participants[0]['status'] === 'active', 'Active participant is missing.');
    ...
    $expect(array_column($activity, 'event_type') === ['access.revoked', 'access.granted'], 'Access history is incomplete or unordered.');
    ...
    echo "VIA-30 local integration: OK\n";
} finally {
    if ($courseId > 0) {
